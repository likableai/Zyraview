...
export async function GET(request: NextRequest) {
  try {
    console.log('🔍 CEX addresses route called, SERVER_URL:', SERVER_URL);
    
    const response = await fetch(`${SERVER_URL}/api/ecosystem?type=cex-addresses`, {
      method: 'GET',
      headers: {
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: 'Failed to fetch CEX addresses',
